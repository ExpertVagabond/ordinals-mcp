...
export const getRuneBalances: McpAction = {
  tool: {
    name: "get_rune_balances",
    description:
  ...
  handler: async (request) => {
    ...
    try {
      if (config.hiro.key) {
        const data = await hiro.getRuneBalances(address, offset, limit);
    ...
    try {
      if (config.ordiscan.key) {
        const data = await ordiscan.getAddressRunes(address, offset, limit);
        return textResult(data);
      }
    } catch (e) {
